# -*- coding: utf-8 -*-
import sys
from PySide2.QtWidgets import QApplication, QWidget, QSpinBox, QDoubleSpinBox, QHBoxLayout
...
5. QSpinBox为整型数字调节框，而QDoubleSpinBox为浮点型数字调节框。
    QDoubleSpinBox的默认范围为0.00-99.99，而小数位数默认是两位，不过可以通过setDecimals(int)方法来设置小数位数；

6-7. 该槽函数主要是在QSpinBox数值发生变化时，将QDoubleSpinBox的整数部分设置成QSpinBox的值，小数部分保持不变。
    所以要首先获取QDoubleSpinBox的小数部分再进行设置。通过setValue()方法可以设置调节框的值，而value()方法是获取值。
"""
